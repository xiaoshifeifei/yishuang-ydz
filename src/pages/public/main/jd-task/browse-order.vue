    ...
    <div id="browse-order">
        ...
        <uiScroll class="wrapper" ref="scroll" :bounce="bounce" :backcolor="'#f5f5f5'" :pullup='true' >
             <main class="main">
                 ...
                 <div class="detail">
                     <div class="top">
                        <uiMosaic class="mosaic"  :image="order.goods_image" @load='getLoad()'></uiMosaic>
                        <div class='right'>
                            <p>佣金&ensp;<span>¥{{order.commission_buyer}}</span></p>
                            <p>类型&ensp;{{ordertype}}</p>
         ...
         <div class="footer giveup">
            <el-button  class="el-button" @click="giveup()"><span>放弃</span></el-button>
         </div>
        <div class="footer accept">
            <el-button  class="el-button"  @click="accept()"><span>接受</span></el-button>
         </div>
    </div>
</template>
<script>
...
export default {
    ...
    created(){
        ...
        MainService.jd_grab_detail({
            order_id:this.order_id
        }).then(res=>{
            if(res.code == 1){
                util.closeLoading();
                this.order = res.data.orderfind;
                this.ordertype = res.data.ordertype;
                const time1 = parseInt(new Date().getTime()/1000);
                this.timer_n = this.order.order_end_time - time1>0?this.order.order_end_time - time1:0;
    ...
    methods:{
        ...
        accept(){
            clearInterval(this.timer);
            MainService.jdaccept_order()
            .then(res=>{
                if(res.code == 1){ 
                    this.$router.push('/jdorder-verify/'+this.order_id+'?type='+this.type);
                }else{
                    util.alert({
                        content:res.msg,
                        define:()=>{
