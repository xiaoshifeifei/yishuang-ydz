...
export default {
    ...
    mounted(){
         document.getElementById('tb-browse').style.height = document.getElementById('tb-browse').offsetHeight - this.$store.state.safeTop +'px';    
    },
    methods:{
        get_record(){
            if(this.page <= this.last_page){
                 CommerceService.get_browseList({
                    page:this.page,
                    ...
                    if(res.code == 1){
                        ...
                        res.data.data.forEach(item => {
                            ...
                            if(item.pass){
                                if(item.pass == 1){
                                   item.judeg_to = '/order-verify/'+item.id+'?type=2&step=1';
                                }else if(item.pass == 2){
                                   item.judeg_to = '/answer-question/'+item.id+'?type=2&step=1';
                                }else if(item.pass == 3){
                                   item.judeg_to = '/pay-commit/'+item.id+'?type=2&step=1';
                                }else if(item.pass == 4){
