    ...
    <div id="running-water">
        <div id="content">
                    ...
                    <main class="record" v-if="records.length>0">
                        <div class="record-list" v-for="(item,index) in records" :key="index" @click="go_flow(item.id)">
                            ...
                            <div class="record-center">
                                    <div class="record-title" v-if="item.order_number!=''">
                                       {{item.order_number}}
                                    </div>
                                    <div class="record-title" v-else>
                                       {{item.bill_number}}
                                    </div>
                                     <div class="record-detail" v-if="item.type!='转出'">
                                        [{{item.type}}]
                                    </div>
            ...
            <div class="settle_accounts" id="settle_accounts" @touchmove.prevent>
                <div class="yet" :class="{'hove_xs':status == 1}" @click="change_status(1)">已审结账</div>
                <div class="not" :class="{'hove_xs':status == 2}" @click="change_status(2)">未审结账</div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created(){
        ...
        if(this.$route.params.id == 1){
            this.title_name = '淘宝账单';
            this.type = 1;
            this.get_record();
            this.path_lj = '/tb-flow/';
        }else{
            this.title_name = '京东账单';
            this.type = 2;
            this.get_record();
            this.path_lj = '/jd-flow/';
        }
        
    },
    mounted(){
         this.$refs.scroll.$el.style.height = document.getElementById("running-water").offsetHeight - this.$refs.title.$el.offsetHeight - document.getElementById("settle_accounts").offsetHeight + 'px';   
    },
    methods:{
        get_record(){
             if(this.page <= this.last_page){
                 FundService.get_tb_record({
                    status:this.status,
                    page:this.page,
